...
$objTpl->getActiveSheet()->setCellValue('C2', stripslashes('RECORD OPL SYSTEM CG ('.$nama.')'));
// Add some data
$query3 = "SELECT COUNT(opl.tema_opl) AS total FROM OPL 
	...
	where opl.status = 7 and opl.tgl_approve_koordinator>='$a' and  opl.tgl_approve_koordinator<='$b' and circle_group.nama_cg='$nama'";
...
$x=$v+2;
$objTpl->getActiveSheet()->setCellValue('B'.$x.'', stripslashes('TOTAL'));
$objTpl->getActiveSheet()->setCellValue('D'.$x.'', stripslashes(''.$row3['total']));
